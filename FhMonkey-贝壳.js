...
(() => {
    ...
    if (pathname.startsWith('/ershoufang/')) {
        var total = 0;
        ...
        if (box) {
            ...
            var target = document.querySelector(
                '.introContent .content li:nth-child(2)'
            );
            var current = parseFloat(target.innerText.match(/\d+(\.\d+)?/)[0]);
            target.appendChild(
                document.createTextNode(
                    `/ ${total}㎡ | ${Number(
                        (((current - total) / current) * 100).toFixed(2)
                    )}%`
                )
            );
        }
    ...
    if (pathname.startsWith('/xiaoqu/')) {
        var cjjl = document.querySelector('#frameDeal .frameDealHeader');
        if (cjjl) {
            cjjl.innerHTML = `<a href='/chengjiao/c${location.pathname.split('/')[2]
                }/' target='_blank'>${cjjl.innerText}</a>`;
        }
    }
})();
